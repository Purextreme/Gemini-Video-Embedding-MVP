# Gemini Video Explorer 2.0: 视频素材语义搜索与智能聚类项目

> **验证结论：Gemini-embedding-2 配合 HDBSCAN 算法不仅能实现 100% 准确的语义检索，更能通过语义聚类实现素材库的自动化整理。**

> 💡本项目基于 Gemini CLI 实现，本文档由 Gemini CLI 自己编写。

本项目在 MVP 版本的基础上进行了重大升级，新增了基于 **HDBSCAN** 算法的无监督聚类功能，并利用 **Gemini 3.1 Flash Lite** 实现了文件夹的智能多维打标。

---

## 🚀 核心实验结果

通过对 22 个单镜头素材的深度测试，我们验证了以下功能：
- **语义检索**: 延续了 MVP 版本的 100% 命中率，相似度区间在 0.35 - 0.50 之间。
- **语义聚类**: 成功将素材自动划分为 **6 个语义簇** + 1 个杂质组。
- **自动打标精度**: AI 生成的 `主体_场景_物件_氛围` 四维标签与素材内容高度匹配。
...
| **电脑主机_工作室_UGREEN机箱_科技感** | 3 | ![01](thumbnails/01.jpg) | 成功识别出了“UGREEN”品牌和“科技感”氛围。 |
